    ...
    int *total)
{
	...
	if(frag[MSG_MTYPE] =='\x01' && frag[MSG_DIR] == '\x00' &&
	   memcmp(frag+12,"\x30\x00\x00\x30",4)) {
		...
		update_status("Battery exhausted, camera off.");
		receive_error=ERROR_LOWBATT;
	}
	else {
	  debug_message("ERROR: unexpected message.\n");
	  update_status("ERROR: unexpected message");
	}
	return NULL;
    }
    frag += MSG_HDR_LEN;
	...
	if (msg_pos+len > length) {
	    ...
	    update_status("ERROR: message overrun");
	    return NULL;
	}
	...
	if (type == PKT_EOT) {
		...
		if (type == PKT_MSG && receive_error == ERROR_ADDRESSED) {
    		...
    		if (len < MSG_HDR_LEN || frag[MSG_02] != 2) {
				debug_message("ERROR: message format error\n");
				update_status("ERROR: message format error.");
				return NULL;
    		}
			...
			if(frag[MSG_MTYPE] =='\x01' && frag[MSG_DIR] == '\x00' &&
			  ...
			  update_status("ERROR: unexpected message2.");
			}
			return NULL;
    		}
    		frag += MSG_HDR_LEN;
...
int psa50_get_owner_name(void)
{
  ...
  memcpy(camera.firmwrev,(char *) msg+8,4);
...
int psa50_ready()
{
    ...
    case CANON_USB:
      ...
      psa50_get_time();
      if (!strcmp("Canon PowerShot S20",camera.ident)) {
	update_status("Detected a Powershot S20");
	camera.model = CANON_PS_S20;
	A5 = 0;
	return 1;
      }
      else if (!strcmp("Canon PowerShot S10",camera.ident)) {
