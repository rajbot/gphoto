...
void on_propertybox_camera_properties_apply (GnomePropertyBox *propertybox, gint arg, gpointer user_data);
gint on_propertybox_camera_properties_close (GnomeDialog *dialog, gpointer user_data);

gboolean setting_get (GladeXML *xml_camera_properties, CameraSetting *setting, CameraWidget *camera_widget);
gboolean camera_properties_set (GladeXML *xml_camera_properties);
void page_entry_new (GtkWidget *vbox, CameraWidget *camera_widget);
GtkWidget *page_new (GladeXML *xml_camera_properties, CameraWidget *camera_widget);
...
{
	GladeXML *xml_camera_properties;
        GnomePropertyBox *propertybox;

        g_assert (editable != NULL);
        xml_camera_properties = gtk_object_get_data (GTK_OBJECT (editable), "xml_camera_properties");
        g_assert (xml_camera_properties != NULL);
        propertybox = GNOME_PROPERTY_BOX (glade_xml_get_widget (xml_camera_properties, "propertybox_camera_properties"));
        g_assert (propertybox != NULL);
...
on_adjustment_value_changed (GtkAdjustment *adjustment, gpointer user_data)
{
        ...
        xml_camera_properties = gtk_object_get_data (GTK_OBJECT (adjustment), "xml_camera_properties");
        g_assert (xml_camera_properties != NULL);
        propertybox = GNOME_PROPERTY_BOX (glade_xml_get_widget (xml_camera_properties, "propertybox_camera_properties"));
        g_assert (propertybox != NULL);
...
on_radiobutton_toggled (GtkToggleButton *togglebutton, gpointer user_data)
{
	...
	xml_camera_properties = gtk_object_get_data (GTK_OBJECT (togglebutton), "xml_camera_properties");
	g_assert (xml_camera_properties != NULL);
	propertybox = GNOME_PROPERTY_BOX (glade_xml_get_widget (xml_camera_properties, "propertybox_camera_properties"));
	g_assert (propertybox != NULL);
	...
	if (strcmp (done, "yes") == 0) gnome_property_box_changed (propertybox);
}
...
void on_propertybox_camera_properties_apply (GnomePropertyBox *propertybox, gint arg, gpointer user_data)
{
	...
	g_assert (propertybox != NULL);
	xml_camera_properties = gtk_object_get_data (GTK_OBJECT (propertybox), "xml_camera_properties");
	g_assert (xml_camera_properties != NULL);

        if (!camera_properties_set (xml_camera_properties)) {
...
gboolean
setting_get (GladeXML *xml_camera_properties, CameraSetting *setting, CameraWidget *camera_widget)
{
	GtkObject *object;
	...
	object = GTK_OBJECT (glade_xml_get_widget (xml_camera_properties, "propertybox_camera_properties"));
...
gboolean
camera_properties_set (GladeXML *xml_camera_properties)
{
	...
	setting_count = 0;
		...
		if (gp_widget_type (camera_widget_child) == GP_WIDGET_SECTION) {
			for (j = 0; j < gp_widget_child_count (camera_widget_child); j++) {
				...
				if (setting_get (xml_camera_properties, &setting[setting_count], widget)) setting_count++;
			}
		} else {
			
			/* This is an orphan. */
			if (setting_get (xml_camera_properties, &setting[setting_count], camera_widget_child)) setting_count++;
		}
	}
...
page_entry_new (GtkWidget *vbox, CameraWidget *camera_widget)
{
	...
	g_assert (vbox != NULL);
	xml_camera_properties = gtk_object_get_data (GTK_OBJECT (vbox), "xml_camera_properties");
	g_assert (xml_camera_properties != NULL);
	object = GTK_OBJECT (glade_xml_get_widget (xml_camera_properties, "propertybox_camera_properties"));
	g_assert (object != NULL);
	...
	case GP_WIDGET_TEXT:
		...
		/* Store some data. */
		gtk_object_set_data (GTK_OBJECT (entry), "xml_camera_properties", xml_camera_properties);
		gtk_object_set_data (object, gp_widget_label (camera_widget), entry);
	...
	case GP_WIDGET_RANGE:
		...
		/* Store some data. */
		gtk_object_set_data (GTK_OBJECT (adjustment), "xml_camera_properties", xml_camera_properties);
		gtk_object_set_data (object, gp_widget_label (camera_widget), hscale);
	...
	case GP_WIDGET_TOGGLE:
		...
		/* Store some data. */
		gtk_object_set_data (GTK_OBJECT (button), "xml_camera_properties", xml_camera_properties);
		gtk_object_set_data (object, gp_widget_label (camera_widget), button);
	...
	case GP_WIDGET_RADIO:
		...
		for (k = 0; k < gp_widget_choice_count (camera_widget); k++) {
			...
			/* Store some data. */
			gtk_object_set_data (GTK_OBJECT (button), "xml_camera_properties", xml_camera_properties);
			gtk_object_set_data (object, gp_widget_choice (camera_widget, k), button);
			gtk_object_set_data (object, gp_widget_label (camera_widget), button);
		}
		break;
	case GP_WIDGET_MENU:
		...
		/* Store some data. */
		gtk_object_set_data (GTK_OBJECT (GTK_COMBO (combo)->entry), "xml_camera_properties", xml_camera_properties);
		gtk_object_set_data (object, gp_widget_label (camera_widget), combo);
...
GtkWidget *
page_new (GladeXML *xml_camera_properties, CameraWidget *camera_widget)
{
	...
	gtk_object_set_data (GTK_OBJECT (vbox), "xml_camera_properties", xml_camera_properties);
 ...
 */
void
camera_properties (GladeXML *xml, Camera *camera, gchar *camera_name)
{
	GnomePropertyBox *propertybox;
	GladeXML *xml_camera_properties;
	...
	if (gp_camera_config_get (camera, camera_widget) == GP_OK) {
		...
		/* Load the interface. */
		xml_camera_properties = glade_xml_new (GNOCAM_GLADEDIR "gnocam.glade", "propertybox_camera_properties");
		if (xml != NULL) { 
			...
			/* Connect the signals. */
			glade_xml_signal_autoconnect (xml_camera_properties);

			/* Store some data for later use. */
			gtk_object_set_data (GTK_OBJECT (propertybox), "xml_camera_properties", xml_camera_properties);
			...
			for (i = 0; i < gp_widget_child_count (camera_widget); i++) {
				...
				if (gp_widget_type (camera_widget_child) == GP_WIDGET_SECTION) {
					...
					/* Set up the page for this section. */
					page_new (xml_camera_properties, camera_widget_child);
				} else {
					if (!orphans) {
					
						/* Set up a section for orphan widgets. */
						vbox_for_orphans = page_new (xml_camera_properties, NULL);
					}
			...
			gnome_app_error (app, _("Could not fine " GNOCAM_GLADEDIR "gnocam.glade. Check if " PACKAGE " was installed correctly."));
		}
	} else {
