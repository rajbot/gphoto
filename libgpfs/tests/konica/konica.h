 ...
 * (qm200, unsigned long).
 */
int k_init (GPPort *, GPFsErr *);
int k_ping (GPPort *, GPFsErr *);
...
int k_get_io_capability (GPPort *, GPFsErr *,
			 KBitRate *bit_rates, KBitFlag *bit_flags);
int k_set_io_capability (GPPort *, GPFsErr *,
			 KBitRate  bit_rate,  KBitFlag  bit_flags);

int k_erase_all (GPPort *device, GPFsErr *,
		 unsigned int *number_of_images_not_erased);

int k_format_memory_card (GPPort *, GPFsErr *);

int k_take_picture (
	GPPort *device, GPFsErr *,
	int image_id_long,
...
int k_get_preview (GPPort *device, GPFsErr *, int thumbnail,
		   unsigned char **image_buffer,
		   unsigned int *image_buffer_size);


int k_set_protect_status (GPPort *device, GPFsErr *, int image_id_long,
			  unsigned long image_id, int protected);


int k_erase_image (GPPort *device, GPFsErr *, int image_id_long,
		   unsigned long image_id);


int k_reset_preferences (GPPort *device, GPFsErr *);
...
int k_get_date_and_time (GPPort *device, GPFsErr *, KDate *date);
int k_set_date_and_time (GPPort *device, GPFsErr *, KDate  date);
...
int k_get_preferences (GPPort *device, GPFsErr *, KPreferences *preferences);
int k_set_preference  (GPPort *device, GPFsErr *, KPreference   preference,
		       unsigned int value);
...
int k_get_status (GPPort *device, GPFsErr *, KStatus *status);
...
int k_get_information (GPPort *device, GPFsErr *, KInformation *info);
...
/* Localization */
int k_localization_tv_output_format_set (GPPort *device, GPFsErr *,
					 KTVOutputFormat tv_output_format);
int k_localization_date_format_set      (GPPort *device, GPFsErr *,
					 KDateFormat date_format);
int k_localization_data_put             (GPPort *device, GPFsErr *,
					 unsigned char *data,
					 unsigned long data_size);

int k_cancel (GPPort *device, GPFsErr *, KCommand *command);
