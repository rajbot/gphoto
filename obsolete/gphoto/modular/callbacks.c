...
void set_camera (char *model) {
	GModule *m;
	char *path,*modname,*camname=NULL,*structname;
	int i=0;
 	...
 	while (strlen(cameras[i].name) > 0) { 
 		if (strcmp(model, cameras[i].name) == 0) { 
		  gtk_label_set(GTK_LABEL(library_name), 
 					cameras[i].name); 
		  camname=strdup(cameras[i].libname);
		  structname=strdup(cameras[i].structname);
 		} 
 		i++; 
 	} 

	if (camname==NULL) {
	  error_dialog("No camera library found.");
	  return;
	};
	/* Ripped this out of gdk_imlib's modules.c */

	/* Build the library module name */
	modname = g_strconcat ("gphoto_", camname, NULL);

	/* this path should be a definable variable....*/
	path = g_module_build_path ("/usr/share/gphoto/drivers", modname);
	...
	if (!m) { 
	  error_dialog("Could not load camera library.");
	  return;
	};

	if (g_module_symbol (m,structname,(void **) &Camera)) {
	  (*Camera->initialize)();
	  return;
	};

	error_dialog("Error in camera library.");
