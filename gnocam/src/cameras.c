...
void folders_delete_recursive (GladeXML* xml, GtkWidget* item);
...
void
folders_delete_recursive (GladeXML* xml, GtkWidget* item)
{
	g_assert (xml != NULL);
	g_assert (item != NULL);

	g_warning ("Dirty code: Not correctly cleaned up!");
	//FIXME: Clean up all subtrees!
}
...
cameras_update (GladeXML* xml, GConfValue* value)
{
	...
	/* Investigate if the new cameras are in the tree. */
        ...
        for (j = 0; j < g_list_length (tree_list); j++) {
			...
			folders_delete_recursive (xml, item);
