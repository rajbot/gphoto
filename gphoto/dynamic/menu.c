...
*/
	...
	/* Camera Menu --------------------------------------------- */
	menu = gtk_menu_new ();
	add_to_menu_acc("Get Index", GTK_SIGNAL_FUNC(getindex),
		    (gpointer)"Get Index", menu,'i',GDK_CONTROL_MASK);
	add_to_menu_acc("Get Empty Index", GTK_SIGNAL_FUNC(getindex_empty),
		    (gpointer)"Get Empty Index", menu,'e',GDK_CONTROL_MASK);
