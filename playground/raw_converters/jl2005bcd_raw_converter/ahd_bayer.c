	...
	for (x = 0; x < w; x++)
	{
		...
		for (color = 0; color < 3; color += 2) {
			if ((color == RED && bayer == pos_code[3])
				...
				value = value2 = div = 0;
				if (x > 0 && y > 0) {
					value += image_h[AD(x - 1, 0, w)
								    + color]
						-image_h[AD(x - 1, 0, w)
								    + GREEN];
					value2+= image_v[AD(x - 1, 0, w)
								    + color]
						-image_v[AD(x - 1, 0, w)
								    + GREEN];
					div++;
				...
				if (x > 0 && y < h - 1) {
					value += image_h[AD(x - 1, 2, w)
								    + color]
						-image_h[AD(x - 1, 2, w)
								    + GREEN];
					value2+= image_v[AD(x - 1, 2, w)
								    + color]
						-image_v[AD(x - 1, 2, w)
								    + GREEN];
					div++;
				...
				image_v[AD(x, 1, w) + color] =
					...
					value = value2 = div = 0;
				if (x > 0) {
					value += image_h[AD(x - 1, 1, w)
								    + color]
						-image_h[AD(x - 1, 1, w)
								    + GREEN];
					value2 += image_v[AD(x - 1, 1, w)
								    + color]
						-image_v[AD(x - 1, 1, w)
								    + GREEN];
					div++;
		    ...
		    unsigned char *image_v, int w, int h, int y, int *pos_code)
{
	 ...
	 * G(x) = (2 * R(x) +2 * G(x + 1) + 2 * G(x - 1) - R(x - 2)
	 *						 - R(x + 2)) / 4
	 * The estimation on a green-blue row works in the same
	 * way; just replace "R" above by "B".
	 */
	for (x = 0; x < w; x++) {
		bayer = (x&1?0:1) + (y&1?0:2);
		/* pos_code[0] = red. green lrtb, blue diagonals */
		    ...
		    unsigned char * buffer_h, unsigned char *buffer_v, int w)
{
	...
	for (j = 1; j < w - 1; j++) {
		...
		RGBeps = MIN(
			MAX(dRGB(i, i - 3, buffer_h),
						dRGB(i, i + 3, buffer_h)),
			MAX(dRGB(i, i - 3 * w, buffer_v),
						dRGB(i, i + 3 * w, buffer_v))
			);
		/*
...
int gp_ahd_interpolate (unsigned char *image, int w, int h, BayerTile tile)
{
	...
	do_green_ctr_row(image, cur_window_h + 3 * w, cur_window_v + 3 * w,
								w, h, 1, p);
	/*
	...
	for (y = 0; y < h; y++) {
		if(y < h - 3) {
			memcpy (window_v + 15 * w, image + 3 * y * w + 9 * w,
								3 * w);
			memcpy (window_h + 15 * w, image + 3 * y * w + 9 * w,
								3 * w);
		} else {
		...
		if (y<h-2) 
			do_rb_ctr_row(cur_window_h, cur_window_v, w, h,
								y + 2, p);
		/*
